...
class DisplayLogger:
    ...
    def add_log(self, level, message):
        if level not in ['ERROR', 'SUCCESS', 'INFO']:
            return
        if level == 'INFO' and not (message.startswith('Detected plate') or message in ['Gate closed', 'Plate exited']):
            return
...
def get_paid_record(plate):
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute('''
            SELECT no, due_payment FROM vehicle_log
            WHERE car_plate = ? AND (payment_status = '1' OR due_payment = 0)
...
def log_unauthorized_attempt(plate, due_payment):
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO unauthorized_exit_alerts (car_plate, alert_time, due_payment, alert_type, resolved, notes)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            plate,
        ...
        conn.commit()
        display_logger.add_log('ERROR', f'Unauthorized exit attempt: {plate}')
    except Exception as e:
        display_logger.add_log('ERROR', f'Failed to log unauthorized exit: {str(e)}')
    finally:
        conn.close()

def update_exit_log(record_id, plate):
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute('''
            UPDATE vehicle_log
            SET exit_time = ?, is_exited = '1'
            WHERE no = ?
        ''', (
            time.strftime('%Y-%m-%d %H:%M:%S'),
            record_id
        ...
        conn.commit()
        display_logger.add_log('SUCCESS', f'Plate {plate} exited successfully')
        system_status['last_action'] = f'Exited {plate}'
    except Exception as e:
        display_logger.add_log('ERROR', f'Failed to update exit log: {str(e)}')
    finally:
        conn.close()

def signal_handler(sig, frame):
    display_logger.add_log('INFO', 'System shutdown')
    sys.exit(0)
...
def main():
    ...
    cv2.namedWindow('Parking System Exit Point', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('Parking System Exit Point', 800, 600)
    ...
    try:
        while True:
            ret, frame = cap.read()
            if not ret:
                display_logger.add_log('ERROR', 'Failed to read frame from camera')
                    ...
                    try:
                        ...
                        if len(plate_buffer) >= CAPTURE_THRESHOLD:
                            ...
                            if common != last_processed_plate or (now - last_exit_time) > EXIT_COOLDOWN:
                                record = get_paid_record(common)
                                if record:
                                    record_id, due_payment = record
                                    update_exit_log(record_id, common)
                                    if arduino:
                                        try:
                                            arduino.write(b'1')
                                    ...
                                    try:
                                        ...
                                        cursor = conn.cursor()
                                        cursor.execute('''
                                            SELECT due_payment FROM vehicle_log
                                            WHERE car_plate = ? AND payment_status = '0'
                                            ORDER BY entry_time DESC LIMIT 1
                                        ''', (common,))
                                        row = cursor.fetchone()
                                        due_payment = row[0] if row else 0.0
                                        conn.close()
                                        log_unauthorized_attempt(common, due_payment)
                                        if arduino:
                                            try:
                                                arduino.write(b'2')
                                            except Exception as e:
                                                display_logger.add_log('ERROR', f'Buzzer control error: {str(e)}')
                                    except Exception as e:
                                        display_logger.add_log('ERROR', f'Database query failed: {str(e)}')
            ...
            cv2.imshow('Parking System Exit Point', annotated)
        ...
        display_logger.add_log('INFO', 'System shutdown complete')
